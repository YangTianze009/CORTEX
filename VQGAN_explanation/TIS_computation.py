...
def compute_contribution(test_loader, model_path, device, results_dir, selected_labels):
    ...
    print(f'Token Importance Score saved to individual label CSVs in {results_dir}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_type', type=str, choices=["train", "test"], default="train",
                        help='Choose which dataset to use: "train" or "test"')
    ...
    data_type = args.data_type # data_type = train or test
    test_csv = f'{data_type}_embeddings.csv'
    # test_csv = 'test_embeddings.csv'
    val_csv = 'validation_embeddings.csv'
    data_folder = 'datasets/'
    batch_size = args.batch_size
    model_path = f"checkpoints/ClassificationNet{args.model}/best_model.pth"
    results_dir = f"results/TIS/Net{args.model}/TIS_value_{data_type}"
    os.makedirs(results_dir, exist_ok=True)
