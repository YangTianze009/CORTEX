'''
This code is used to generate statistics information based on token importance score (TIS). Such as which token has the largest TIS and how many times it occurs in specific label
'''
...
parser.add_argument('--model', type=int, choices=[1, 2, 3, 4], default=1,
    ...
    with tqdm(total=file_count, desc="Processing CSV files", unit="file") as pbar:
        for csv_file in csv_files:
            ...
            for row in csv_data:
                ...
                for n in top_n_list:
                    top_n_indices = np.argsort(contribution_values)[-n:][::-1]  # Get token indices with highest TIS
            ...
            # save the result for each label
...
if __name__ == "__main__":
    ...
    folder_path = f"results/TIS/Net{args.model}/TIS_value_train"  
    save_root = f"results/TIS/Net{args.model}/TIS_statistics_train"  
    embedding_csv_path = "datasets/train_embeddings.csv"  
